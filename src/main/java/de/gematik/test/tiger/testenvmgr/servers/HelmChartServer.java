 ...
 * It starts a helm chart on your local / remote kubernetes cluster using the {@link KubeUtils} helper.
 */
...
public class HelmChartServer extends AbstractExternalTigerServer {
    ...
    public void assertThatConfigurationIsCorrect() {
        ...
        if (options.getHealthcheckPods() == null) {
            log.warn("Detected empty healthcheckPod list, adding podName {} as default entry", getHelmChartOptions().getPodName());
    ...
    public void performStartup() {
        ...
        setStatus(TigerServerStatus.STARTING, "Starting helm chart for " + getServerId() +
            " from " + getConfiguration().getSource().get(0) + " as pod "
        ...
        if (exitCode != 0) {
            ...
            throw new TigerTestEnvException(FAILED_START_MESSAGE + " %s! Please check log!",
                getServerId());
        } else {
            waitForServerUp();
            logAllPods();
    ...
    public TigerServerStatus updateStatus(boolean quiet) {
        try {
            ...
            setStatus(
                kubeUtils.getNumOfRunningPods(getHelmChartOptions().getNameSpace()) == getHelmChartOptions().getHealthcheckPods().size() ?
                    TigerServerStatus.RUNNING : TigerServerStatus.STARTING);
            return getStatus();
        } catch (TigerTestEnvException ttException) {
            RuntimeException ex = new TigerTestEnvException("Unable to look up kubernetes pods for helm chart " + getServerId(),
                ttException);
            if (!quiet) {
    ...
    public void shutdown() {
        ...
        if (getConfiguration().getHelmChartOptions().getPodName() == null) {
            log.warn("Helm chart pod name not specified in tiger.yaml under helmChartOptions -> podName. "
        ...
        final Optional<CompletableFuture<Process>> shutdownFuture = kubeUtils.shutdownHelm(getHelmChartOptions().getNameSpace());
    ...
    private void waitForShutdownToComplete() {
        if (getHelmChartOptions().getHealthcheckPods().isEmpty()) {
            ...
            try {
                ...
                await().atMost(getConfiguration().getStartupTimeoutSec(), TimeUnit.SECONDS)
                    .pollInterval(1, TimeUnit.SECONDS)
                    .until(() -> {
                        try {
                            long runningPods = kubeUtils.getNumOfPodsOnStatusList(getHelmChartOptions().getNameSpace());
                            if (getHelmChartOptions().isDebug() && runningPods != 0) {
                                log.info("{} pods of helm chart {} still present, waiting {}s ", runningPods, getServerId(),
                ...
                throw new TigerTestEnvException("Timeout waiting for helm chart server %s shutdown!", getServerId());
            }
        }
    }
