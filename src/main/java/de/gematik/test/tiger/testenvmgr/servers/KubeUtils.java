...
/**
 * Bundles all functionality about running helm und kubtctl calls directly on OS level (using {@link
 * ProcessBuilder}).
 */
public class KubeUtils {

  public static final String PORT_EXCEPTION_MESSAGE =
      "Failed to start kubectl port forward for command ";

  /**
   * full path to the helm command found in one of the folders of the PATH env variable on your
   * local system or null.
   */
  private final String helmCommand;

  /**
   * full path to the kubectl command found in one of the folders of the PATH env variable on your
   * local system or null.
   */
  ...
  /**
   * current working directory the kubectl / helm commands should be working out of. Specified in
   * helmChartOption: workingDir
   */
  @Setter private String workingDirectory;
  ...
  public void setKubernetesContext(String context) {
    ...
    log.info(
        "Setting kubernetes context for helm chart {} to {}...",
        tigerServer.getServerId(),
        context);
    ...
    if (exitCode != 0) {
      throw new TigerTestEnvException(
          ...
          context, tigerServer.getServerId());
    }
  }
            ...
            values -> {
              Optional<String> serviceName =
                  serviceNames.stream()
                      .filter(
                          svcName ->
                              svcName.equals(values.get(0)) || svcName.matches(values.get(0)))
                      .findAny();
              if (serviceName.isPresent()) {
                String podName = serviceName.get();
                log.info(
                    "Exposing ports {} of service {} for helm chart {}...",
                ...
                final ProcessBuilder processBuilder =
                    new ProcessBuilder().command(cmd).inheritIO().redirectErrorStream(true);
  ...
  private List<String> getKubernetesStatusLines(String nameSpace) {
    final ProcessBuilder processBuilder =
        new ProcessBuilder()
            .command(
                kubeCtlCommand, "get", "pods", "-o", "wide", "-n", getNameSpaceOrDefault(nameSpace))
            .redirectErrorStream(true);
    return getSafely(
        spinUpNewExternalProcess(processBuilder)
            .thenApplyAsync(
                process -> {
                  ...
                  try {
                    return Arrays.stream(
                            IOUtils.toString(input, StandardCharsets.UTF_8).split("\n"))
                        .skip(1)
                        .toList();
                  } catch (IOException e) {
                    throw new TigerTestEnvException(
                        "Unable to retrieve list of pods from kubernetes cluster!", e);
                  }
                }),
  ...
  private List<String> getKubernetesServices() {
    final String nameSpace =
        getNameSpaceOrDefault(tigerServer.getConfiguration().getHelmChartOptions());
    final ProcessBuilder processBuilder =
        new ProcessBuilder()
            ...
            .thenApplyAsync(
                process -> {
                  ...
                  try {
                    ...
                    throw new TigerTestEnvException(
                        "Unable to retrieve list of services from kubernetes cluster!", e);
                  }
                }),
  ...
  private void checkKubeCtlPortForwarding(ProcessBuilder processBuilder, String command) {
    ...
    try {
      hasExited =
          getSafely(spinUpNewExternalProcess(processBuilder), "start kubectl port forward")
              .waitFor(1, TimeUnit.SECONDS);
    } catch (InterruptedException e) {
      log.error("Failed to start kubectl port forward - InterruptedException {}", command);
      Thread.currentThread().interrupt();
      throw new TigerTestEnvException(
          "Failed to start kubectl port forward - InterruptedException {}", e);
    }
    if (hasExited) {
      log.error(PORT_EXCEPTION_MESSAGE + " {}", command);
      throw new TigerTestEnvException(
          PORT_EXCEPTION_MESSAGE + "\"%s\"! Please check exposedPorts in your tiger.yaml!",
          command);
    }
  }
  ...
  public long getNumOfPodsOnStatusList(String nameSpace) {
    return getKubernetesStatusLines(nameSpace).stream()
        .map(line -> line.split(" +"))
        .filter(
            columns ->
                tigerServer.getConfiguration().getHelmChartOptions().getHealthcheckPods().stream()
  ...
  public Optional<CompletableFuture<Process>> shutdownHelm(String nameSpace) {
    ProcessBuilder processBuilder =
        new ProcessBuilder().command(helmCommand, "list", "-n", getNameSpaceOrDefault(nameSpace));
    applyEnvPropertiesToProcess(processBuilder);
    final CompletableFuture<Process> shutdownFuture = spinUpNewExternalProcess(processBuilder);
    try {
      String list =
          IOUtils.toString(
              getSafely(shutdownFuture, "list helm charts").getInputStream(),
              StandardCharsets.UTF_8);
      if (Arrays.stream(list.split("\n"))
          .noneMatch(
              chartName ->
                  chartName.startsWith(
                      tigerServer.getConfiguration().getHelmChartOptions().getPodName()))) {
        log.warn(
            "Helm chart {} not listed, no need to issue shutdown command!",
            tigerServer.getConfiguration().getHelmChartOptions().getPodName());
        return Optional.empty();
      }
    } catch (IOException e) {
      throw new TigerTestEnvException(
          "Unable to obtain list of helm charts! Trying to shutdown nevertheless.", e);
    }
    ...
    return CompletableFuture.supplyAsync(
            () -> {
              try {
                ...
                throw new TigerTestEnvException(
                    e, "Unable to start helm chart '%s'!", tigerServer.getServerId());
              }
            },
            executor)
        .thenApplyAsync(
  ...
  public <T> T getSafely(CompletableFuture<T> future, String cmdText) {
    try {
      ...
      Thread.currentThread().interrupt();
      throw new TigerTestEnvException(
          e, "Interruption received while executing command %s", cmdText);
    }
  }
    ...
    final ProcessBuilder processBuilder =
        new ProcessBuilder()
            .command(
                ...
                podName,
                "-n",
                tigerServer.getConfiguration().getHelmChartOptions().getNameSpace(),
                "-f")
            .redirectErrorStream(true);
    spinUpNewExternalProcess(processBuilder)
        .thenAccept(
            process -> {
              ...
              new TigerStreamLogFeeder(
                  "Pod " + podName, log, process.getErrorStream(), Level.ERROR);
            });
  }
  ...
  private void applyEnvPropertiesToProcess(ProcessBuilder processBuilder) {
    processBuilder
        .environment()
        .putAll(
            tigerServer.getEnvironmentProperties().stream()
                .map(str -> str.split("=", 2))
    ...
    for (Process process : processes) {
      if (process.isAlive()) {
        if (log.isInfoEnabled()) {
          log.info(
              "Destroying process calling kubernetes/helm {} ({})",
              process.pid(),
